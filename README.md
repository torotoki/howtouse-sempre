...
The `run` script is the command-line interface for SEMPRE. You have to specify a running mode  from the following list:
```
Modes:
  test: Run unit tests
...
You can use it for learning data, executing denotations, testing and others with following options:
...
Or you can run TestNG directly to specify detail options:
```bash
...
You can specify include/exclude tests on `testing.xml`:


Note: it takes a few time to load the entire package even if you run only one test.


### Testing specific methods
If you only want to test specific methods, you should specify the couple of methods belong to a new group with an annotation:
```java
@Test(groups = {"nulltest"})
public void testMethod() {...}
```
Then, you should also specify this group to be included (or excluded) on `testng.xml`:
```xml
<groups>
  <run>
